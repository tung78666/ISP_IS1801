...
<html>
    <head>
        ...
        <script type="text/javascript">
            function checkDelete(id, bok) {
                id = id.trim();
                bok = bok.trim();
                if (confirm('ARE YOU SURE DELETING BORROW ID: ' + id + ',UNDER BOOK ID: ' + bok + '?')) {
                    window.location = 'deleteBorrowBook?id=' + id + '&bok=' + bok;
                }
            }
        </script>
    </head>
            ...
            <table style="width: 100%" id="border" >
                ...
                <c:forEach items="${requestScope.bList}" var="b">
                    <tr>
                        ...
                        <td>${b.getQuantity()}</td>
                        <td>
                            <form action="requestBorrow" method="get">
                                <input type="hidden" name="renewOrderBorrowID" value="${b.getBorrowID()}">
                                <input type="hidden" name="renewOrderBookID" value="${b.getBook().getBookID()}">
                                <input type="submit" class="btn btn-danger" value="Renew">
                            </form>
                        </td>
                        <td>
                            <input type="submit" class="btn btn-danger" value="Delete" onclick="checkDelete('${b.getBorrowID()}', '${b.getBook().getBookID()}')">
                        </td>
                    </tr>
                </c:forEach>
            </table>
